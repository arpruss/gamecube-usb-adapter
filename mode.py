...
def sendCommand(command):
    data = [reportId] + list(map(ord, command))
...
while myReport is None:    
    ...
    if myReport is None:
        for d in hid.HidDeviceFilter(vendor_id = 0x0F0D, product_id = 0x00c1).get_devices():
            device = d
            device.open()

            for report in device.find_feature_reports():
                if report.report_id == 0 and report.report_type == "Feature":
                    myReport = report
                    break
